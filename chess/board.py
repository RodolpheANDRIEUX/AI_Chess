...
def initialize_board():
    grid = np.full((8, 8), None, dtype=object)
    for i in range(8):
...
class Board:
    def __init__(self):
        ...
        self.valid_moves = np.full((8, 8), False)
        self.selected_piece = None
        self.last_move = None

    def move_piece(self, move):
        self.pieces[move.end_pos] = move.piece
        self.pieces[move.start_pos] = None
        move.piece.position = move.end_pos

    def undo_move(self, move):
        self.pieces[move.start_pos] = move.piece
        self.pieces[move.end_pos] = None
        move.piece.position = move.start_pos

    def play(self, i, j):
        move = Move(self.selected_piece.position, (i, j), self.selected_piece)
        print(f"{self.selected_piece.position}")
    ...
    def king_safe(self, move):
        original_start_piece = self.pieces[move.start_pos[0], move.start_pos[1]]
        original_end_piece = self.pieces[move.end_pos[0], move.end_pos[1]]
        safe = True

        # Move the piece
        self.move_piece(move)
        for nx, ny in np.ndindex(self.pieces.shape):
            ...
            if piece is not None and piece.color != move.piece.color:
                ...
                for row, col in np.ndindex(valid_enemy_moves.shape):
                    if not valid_enemy_moves[row, col] == False and self.get_piece_at(row, col) is not None:
        ...
        self.pieces[move.start_pos[0], move.start_pos[1]] = original_start_piece
        self.pieces[move.end_pos[0], move.end_pos[1]] = original_end_piece
        return safe
        ...
        for i in range(8):
            for j in range(8):
                ...
                if piece is not None and piece.color == self.selected_piece.color:  # for every piece of the current p
                    ...
                    for row in range(8):
                        for col in range(8):
                            if not valid_moves[row, col] == False and self.get_piece_at(row, col) is not None:  # for
        ...
        for i in range(8):
            for j in range(8):
                ...
                if piece is not None and piece.color == enemy:
                    ...
                    for row in range(8):
                        for col in range(8):
                            if not valid_moves[row, col] == False:
